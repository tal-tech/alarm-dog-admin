...
class CycleCompareController extends AbstractController
{
    /**
     * @Inject
     * @var MonitorCycleCompare
     */
    protected $task;
    ...
    public function list()
    {
        $param = $this->validate([
            ...
            'status' => 'nullable|integer|in:' . implode(',', array_keys(MonitorCycleCompare::$statuses)),
    ...
    public function store()
    {
        $param = $this->validate([
            ...
            'agg_cycle' => 'required|integer|in:' . implode(',', MonitorCycleCompare::$supportedAggCycles),
            'compare_cycle' => 'required|integer|in:' . implode(',', MonitorCycleCompare::$supportedCompareCycles),
     ...
     * 通过数据源初始化数据.
     */
    public function dataInitByDatasource()
    {
        $param = $this->validate([
            'id' => 'required|integer',
        ]);

        $resp = $this->task->dataInitByDatasource($param['id']);
     ...
     * 通过Push方式初始化数据.
     */
    public function dataInitByPush()
    {
        $param = $this->validate([
            'id' => 'required|integer',
        ...
        $resp = $this->task->dataInitByPush($param['id'], $param['access_token'], $param['data']);
     ...
     * 通过webhook方式初始化数据源.
     */
    public function dataInitByWebhook()
    {
        $param = $this->validate([
            'id' => 'required|integer',
        ...
        $resp = $this->task->dataInitByWebhook($param['id'], $param['webhook']);
